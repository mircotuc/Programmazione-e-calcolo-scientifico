...
public:
    ...
    std::ostream& operator<<(std::ostream& os, const unidirected_edge<T>& edge) {
        os << "(" << edge.start_node << "," << edge.end_node << ")";
        return os;
    }
};
...
private:
    ...
    // I use unordered_map because it is faster when searching in it than map, and I don't care about the order
...
public:
    ...
    unidirected_graph<T> operator-(const unidirected_graph<T>& other) const {
        unidirected_graph<T> difference_graph; //this will be my result
...
template<typename T>
std::ostream& operator<<(std::ostream& os, const unidirected_graph<T>& graph)
{
